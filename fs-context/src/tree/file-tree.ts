...
async function buildNode(
  ...
  try {
    ...
    filteredEntries = entries
      ...
      .filter((entry) => {
        ...
        if (entry.name.startsWith('.') && entry.name !== '.ai-team' && !ctx.includeHidden)
          return false;
        return true;
      })
      .sort((a, b) => {
  ...
  const ruleSets = ctx.ignoreGitignore
    ? []
  ...
  const childNodes = await Promise.all(
    filteredEntries.map(async (entry) => {
      ...
      const { include, gitignored } = await resolveGitignored(
        childRel,
        entry.isDirectory,
        ruleSets,
        ctx
...
async function collectFlat(
  ...
  const nested = await Promise.all(
    filteredEntries.map(async (entry) => {
      ...
      const childResults = await collectFlat(
        childAbs,
        workspaceRoot,
        depth + 1,
        maxDepth,
