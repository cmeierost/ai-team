...
import { emitFileEdited, removeAllFileEventListeners } from '../fs/file-events.js';
...
describe('findFormatters', () => {
  it('returns matching enabled formatters by extension', async () => {
    const result = await findFormatters('/project/src/app.ts', testFormatters);
    expect(result.map((f) => f.name)).toEqual(['fake-prettier']);
  });

  it('returns empty array for unknown extensions', async () => {
    expect(await findFormatters('/project/src/data.bin', testFormatters)).toEqual([]);
  });

  it('excludes disabled formatters', async () => {
    const result = await findFormatters('/project/src/app.ts', testFormatters);
    expect(result.map((f) => f.name)).not.toContain('disabled');
  });

  it('returns empty array for files without extensions', async () => {
    expect(await findFormatters('/project/Makefile', testFormatters)).toEqual([]);
  });
});
...
describe('getFormatterStatus', () => {
  it('returns enabled/disabled status of all formatters', async () => {
    const status = await getFormatterStatus(testFormatters);
...
describe('BUILT_IN_FORMATTERS integration', () => {
  it('built-in prettier is disabled when prettier is not on PATH', async () => {
    ...
    expect(await prettier!.enabled()).toBe(false);
  });

  it('built-in prettier is enabled when which returns a path', async () => {
    mockWhich.mockResolvedValue('/usr/bin/prettier');
    const { BUILT_IN_FORMATTERS } = await import('./format.js');
    const prettier = BUILT_IN_FORMATTERS.find((f) => f.name === 'prettier');
    expect(await prettier!.enabled()).toBe(true);
  });
});
