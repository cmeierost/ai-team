...
import { onFileEdited, type FileEditedEvent } from '../fs/file-events.js';
...
export const BUILT_IN_FORMATTERS: readonly FormatterInfo[] = [
  {
    ...
    enabled: async () => (await which('prettier')) !== null,
  },
  {
    name: 'biome',
    command: ['biome', 'check', '--write', '$FILE'],
    extensions: ['.js', '.jsx', '.mjs', '.cjs', '.ts', '.tsx', '.mts', '.cts', '.json', '.jsonc'],
    enabled: async () => (await which('biome')) !== null,
  },
  {
    name: 'gofmt',
    command: ['gofmt', '-w', '$FILE'],
    extensions: ['.go'],
    enabled: async () => (await which('gofmt')) !== null,
  },
  {
    name: 'rustfmt',
    command: ['rustfmt', '$FILE'],
    extensions: ['.rs'],
    enabled: async () => (await which('rustfmt')) !== null,
  },
  {
    name: 'ruff',
    command: ['ruff', 'format', '$FILE'],
    extensions: ['.py', '.pyi'],
    enabled: async () => (await which('ruff')) !== null,
  },
  {
    name: 'clang-format',
    command: ['clang-format', '-i', '$FILE'],
    extensions: ['.c', '.cc', '.cpp', '.cxx', '.h', '.hh', '.hpp', '.hxx'],
    enabled: async () => (await which('clang-format')) !== null,
  },
  {
    name: 'shfmt',
    command: ['shfmt', '-w', '$FILE'],
    extensions: ['.sh', '.bash'],
    enabled: async () => (await which('shfmt')) !== null,
  },
  {
    name: 'dart',
    command: ['dart', 'format', '$FILE'],
    extensions: ['.dart'],
    enabled: async () => (await which('dart')) !== null,
  },
  {
    name: 'zig',
    command: ['zig', 'fmt', '$FILE'],
    extensions: ['.zig', '.zon'],
    enabled: async () => (await which('zig')) !== null,
  },
  {
    name: 'mix',
    command: ['mix', 'format', '$FILE'],
    extensions: ['.ex', '.exs'],
    enabled: async () => (await which('mix')) !== null,
  },
];
 ...
 */
export async function findFormatters(
  filePath: string,
  formatters: readonly FormatterInfo[] = BUILT_IN_FORMATTERS
): Promise<FormatterInfo[]> {
  const ext = path.extname(filePath).toLowerCase();
  if (!ext) return [];
  const byExt = formatters.filter((f) => f.extensions.includes(ext));
  const checks = await Promise.all(byExt.map((f) => f.enabled()));
  return byExt.filter((_, i) => checks[i]);
}
...
export async function formatFile(
  ...
  const candidates = await findFormatters(filePath, formatters);
  if (candidates.length === 0) return { formatted: false };

  const formatter = candidates[0]!;
  const args = formatter.command.map((arg) => (arg === '$FILE' ? filePath : arg));
  const bin = args[0]!;
  ...
  try {
    await execFileAsync(bin, rest, {
      timeout: 30_000,
      env: formatter.environment ? { ...process.env, ...formatter.environment } : undefined,
    });
...
export async function getFormatterStatus(
  ...
  const checks = await Promise.all(formatters.map((f) => f.enabled()));
  return formatters.map((f, i) => ({
    name: f.name,
