    ...
    try {
      ...
      if (!agentId || agentId.trim().length === 0) {
        ...
        if (!resolvedAgent) {
          ...
          throw new Error(
            'No agents found in this workspace. Run ait init to initialize your team.'
          );
        }
        writeInfo(
          hooks,
      ...
      let skill;
      try {
        skill = await this.agentDocumentStorage.loadSkillAsync(agent.skillPath);
      ...
      let instructions =
        await this.agentDocumentStorage.loadAllInstructionFilesAsync(workspaceRoot);
