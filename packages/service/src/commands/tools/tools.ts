...
async function resolveFullAgent(
  ...
  const resolved = await agentManager.resolveAgentForOperationAsync(query, operation);
...
function resolveToolIdentifier(
  ...
  // Canonical lookup key (e.g., "hr_hire").
...
export async function allowToolCommand(
  ...
  const resolved = await agentManager.resolveAgentForOperationAsync(options.agent, 'allow tool');
  const agent = await agentManager.getAgentAsync(resolved.id);
  if (!agent) {
    throw new Error(`Agent not found: ${resolved.id}`);
  }
...
export async function disallowToolCommand(
  ...
  const resolved = await agentManager.resolveAgentForOperationAsync(options.agent, 'disallow tool');
  const agent = await agentManager.getAgentAsync(resolved.id);
  if (!agent) {
    throw new Error(`Agent not found: ${resolved.id}`);
  }
