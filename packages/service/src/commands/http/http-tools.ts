...
export interface HttpCrawlParams {
  url: string;
...
function collectRegexSnippets(
  source: string,
  pattern: RegExp,
  contextChars: number,
  maxChunks: number
): { snippets: string[]; count: number } {
...
function applyHttpTextFilters(rawText: string, options: HttpFilterOptions): HttpPreparedText {
  ...
  const startLine = options.startLine
    ? clampNumber(options.startLine, 1, originalLines.length || 1)
    : 1;
  const endLineInput = options.endLine
    ? clampNumber(options.endLine, startLine, originalLines.length || startLine)
...
async function runHttpCrawl(
  ...
  const allowedHosts = new Set(
    [start.hostname, ...options.allowedDomains.map((d) => d.trim()).filter(Boolean)].map((domain) =>
      domain.toLowerCase()
    )
  );
  ...
  while (queue.length > 0 && visited.size < options.maxPages) {
    ...
    const crawled = await crawlSingleHttpPage(
      current.href,
      options.timeoutMsPerPage,
...
async function crawlSingleHttpPage(
  ...
  try {
    const fetched = await fetchUrlText(
      href,
      clampNumber(timeoutMsPerPage, 500, HTTP_MAX_TIMEOUT_MS)
    );
