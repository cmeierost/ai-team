...
describe('dynamic slash catalog', () => {
  it('normalizes slash keys to kebab-case lowercase', () => {
    expect(normalizeSlashKey('Code Review')).toBe('code-review');
    expect(normalizeSlashKey('CODE_review')).toBe('code-review');
    expect(normalizeSlashKey('  --Plan  Alpha--  ')).toBe('plan-alpha');
  });
  ...
  it('skips skill keys that conflict with built-in commands and warns', async () => {
    ...
    const result = await buildDynamicSlashCatalog({
      workspaceRoot,
      skillManager,
      reservedKeys: ['help'],
    });
    ...
    expect(result.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining('conflicts with built-in /help'),
      ])
    );
  });
  ...
  it('chooses newer duplicate skills and warns', async () => {
    ...
    expect(result.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining('Duplicate skill key /code-review'),
      ])
    );
  });
  ...
  it('chooses newer duplicate prompts and falls back to first content line for description', async () => {
    ...
    expect(result.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining('Duplicate prompt key /code-review'),
      ])
    );
  });
  ...
  it('supports configurable prompt/workflow globs and workflow tool naming', async () => {
    const workspaceRoot = await createTempWorkspace();
