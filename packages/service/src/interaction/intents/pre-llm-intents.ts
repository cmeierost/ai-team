...
interface ScoredToolIntentCandidate {
  kind: 'tool';
  toolName: string;
...
interface ScoredCommandIntentCandidate {
  kind: 'command';
  commandKey: string;
  rawArgs?: string;
  score: number;
...
export type ScoredPreLlmIntentCandidate = ScoredToolIntentCandidate | ScoredCommandIntentCandidate;
...
function isExecutableCommandCandidate(
  candidate: ScoredPreLlmIntentCandidate
): candidate is ScoredCommandIntentCandidate {
  return (
    candidate.kind === 'command' &&
    typeof candidate.commandKey === 'string' &&
    candidate.commandKey.trim().length > 0
  );
}

function canExecuteCandidate(candidate: ScoredPreLlmIntentCandidate): boolean {
  if (candidate.kind === 'command') {
    return isExecutableCommandCandidate(candidate);
  }
...
function normalizeCandidates(
  ...
  return candidates
    ...
    .filter((c) => {
      ...
      if (c.kind === 'command') {
        return c.commandKey.trim().length > 0;
      }
...
function formatCandidatePreview(candidate: ScoredPreLlmIntentCandidate): string {
  if (candidate.kind === 'command') {
    return `${candidate.commandKey}${candidate.rawArgs ? ` ${candidate.rawArgs}` : ''}`;
  }
...
function buildHighScoreSelectionIntent(candidates: ScoredPreLlmIntentCandidate[]): PreLlmIntent {
  const toolCandidates = candidates.filter(
    (candidate): candidate is ScoredToolIntentCandidate =>
      candidate.kind === 'tool' && hasInferableArgs(candidate.args)
  );

  const sorted = [...toolCandidates].sort((a, b) => b.score - a.score);
  const options = sorted.map((candidate, index) => ({
    name: `shall i call ${formatCandidatePreview(candidate)}?`,
    value: String(index),
...
function buildConfirmIntent(candidate: ScoredPreLlmIntentCandidate): PreLlmIntent {
  if (candidate.kind !== 'tool' || !hasInferableArgs(candidate.args)) {
    throw new Error('Confirm intent requires a tool candidate with inferable args.');
  }
  ...
  return {
    ...
    ask: {
      kind: 'confirm',
      message: `shall i call ${formatCandidatePreview(candidate)}?`,
      defaultBoolean: true,
    },
    resolveArgs(answer: unknown) {
...
export class PreLlmIntentResolver {
  ...
  async resolve(
    ...
    const highScoreCandidates = candidates.filter(
      (candidate) =>
        candidate.kind === 'tool' &&
        candidate.score >= CONFIRM_THRESHOLD_SCORE &&
        hasInferableArgs(candidate.args)
    );
