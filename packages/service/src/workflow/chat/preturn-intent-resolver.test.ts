...
import { WorkflowPreTurnIntentResolver } from './preturn-intent-resolver.js';
...
describe('WorkflowPreTurnIntentResolver', () => {
  it('auto-selects deterministic score 100 candidate', async () => {
    const resolver = new WorkflowPreTurnIntentResolver(
  ...
  it('returns undefined when no candidate reaches thresholds and no clarification is provided', async () => {
    ...
    const resolver = new WorkflowPreTurnIntentResolver(new TestToolSource([lowScoreTool]));
