...
/**
 * Canonical workflow IDs for chat runtime workflow definitions.
 * Legacy workflow definitions have been migrated to XState-based WorkflowDefinition<T>.
 */
export const chatWorkflowIdSchema = z.enum([]).optional();
...
/**
 * Input to the chat loop orchestration (used by ChatRuntime).
 */
...
/**
 * Output from the chat loop orchestration (used by ChatRuntime).
 */
export interface ChatLoopOutput {
...
/**
 * Input to failure handler step (used by ChatRuntime).
 */
export interface ChatLoopFailureInput {
  error: string;
  hop: number;
  state:
    | 'preturn'
...
/**
 * Legacy re-exports for backward compatibility (use Chat* versions instead).
 */
export type ChatLoopPreturnResult = ChatPreturnResult;
export type ChatLoopToolRoundResult = ChatToolRoundResult;
export type ChatLoopToolCall = ChatToolCall;
export type ChatLoopPostTurnResolutionResult = ChatPostTurnResolutionResult;
export type ChatLoopHandoffTransitionResult = ChatHandoffTransitionResult;
