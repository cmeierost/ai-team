...
/**
 * Evaluates `{{...}}` template expressions against accumulated workflow state.
 *
 * Supports:
 ...
 * - Literal strings without `{{...}}` are returned as-is
 */
...
/**
 * Resolve a JSON-serializable arg/result value using template expressions and
 * declarative transforms (`$literal`, `$coalesce`, `$map`).
 */
...
function evaluateExpression(
  ...
  // If no placeholders, return as literal
  if (!expr.includes('{{')) return parseLiteral(expr);

  // Single placeholder spans the entire expression — return raw value (preserves objects/arrays)
  ...
  // Mixed template — interpolate each placeholder into a string
...
/**
 * Check if resolved params still contain unresolved template expressions.
 */
...
/**
 * Get the list of unresolved parameter names from resolved params.
 */
...
/**
 * Parameter resolver context — services needed for LLM-assisted user questions.
 */
...
/**
 * Resolve workflow step parameters with two-phase resolution:
 * 1. Evaluate template expressions against accumulated state
 * 2. For missing required params, ask user one at a time via LLM-generated questions
 *
 * @param args - Template expressions from workflow step definition
 * @param state - Accumulated workflow state
 * @param toolSchema - Zod schema of the target tool's parameters
 * @param ctx - Services for LLM-assisted questions
 * @param index - Current loop iteration index (if inside a loop)
 * @returns Resolved parameters ready for tool execution
 */
export async function resolveParamsAsync<T>(
  ...
  // Phase 1: Evaluate template expressions
  const resolved = resolveTemplateExpressions(args, state, index);

  // Phase 2: Ask for missing required params one at a time
  // For object schemas, iterate field-by-field; otherwise just validate.
