...
describe('chat-loop-contracts', () => {
  ...
  it('accepts known chat workflow ids', () => {
    expect(chatWorkflowIdSchema.parse('chat-full-loop')).toBe('chat-full-loop');
    expect(chatWorkflowIdSchema.parse('chat-handoff-transition')).toBe('chat-handoff-transition');
  });
