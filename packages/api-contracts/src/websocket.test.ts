...
describe('streamViaWebSocket', () => {
  ...
  it('requests an answer and continues when questions are enabled', async () => {
    ...
    expect(
      parsedMessages.some(
        (message) => message.type === 'answer' && message.answer?.value === 'chosen-value'
      )
    ).toBe(true);
  });

  it('answers mediator-wrapped question events before continuing the stream', async () => {
    const wsInstances: MockWebSocket[] = [];
    const WebSocketMock = vi.fn(function (this: unknown, url: string) {
      const instance = new MockWebSocket(url);
      wsInstances.push(instance);
    ...
    const onQuestion = vi.fn(async () => true);
    const iterator = streamViaWebSocket<any>('agent-1', 'hello', {
      url: 'ws://localhost:3002',
      onQuestion,
    });
    ...
    expect(
      parsedMessages.some(
        (message) => message.type === 'answer' && message.answer?.questionId === 'question-1'
      )
    ).toBe(true);
  });
});
