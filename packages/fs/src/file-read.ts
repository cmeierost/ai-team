...
export async function isBinaryByContent(absolutePath: string, fileSize: number): Promise<boolean> {
  ...
  try { await fd.read(probeBuf, 0, probeBuf.length, 0); }
...
export async function detectContentKind(absolutePath: string, fileSize: number): Promise<FileContentKind> {
...
export async function streamTextFile(
  ...
  try {
    for await (const text of rl) {
      ...
      const line = text.length > maxLineLen
        ? text.substring(0, maxLineLen) + '... (line truncated)'
        : text;
...
export type ReadFileResult =
  ...
  | { kind: 'directory'; entries: string[]; totalEntries: number; offset: number; limit: number; hasMore: boolean }
  | { kind: 'media'; mimeType: string; base64: string; sizeBytes: number }
  | { kind: 'binary'; sizeBytes: number }
  | { kind: 'text'; content: string; totalLines: number; offset: number; limit: number; hasMore: boolean; truncatedByBytes: boolean; nextOffset?: number }
 ...
 */
export async function readFile(absolutePath: string, opts: ReadFileOptions): Promise<ReadFileResult> {
...
async function readTextContent(
  ...
  const { raw, totalLines, truncatedByBytes, hasMoreLines } =
    await streamTextFile(absolutePath, offset, limit);
