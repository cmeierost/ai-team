 ...
 * AgentManager.resolveAgent() and AgentManager.searchAgents().
 *
...
import { Agent, AgentSearchOptions, AgentSearchResult } from '../types/index.js';
 ...
 */
export function filterAndRankAgents(options: AgentSearchOptions, agents: Agent[]): AgentSearchResult[] {
  let filtered = agents;
  ...
  if (options.specialization) {
    ...
    filtered = filtered.filter(a =>
      a.specializations &&
      specs.some(s => a.specializations!.some(as => as.toLowerCase().includes(s.toLowerCase()))),
    );
  }
  if (options.tool) {
