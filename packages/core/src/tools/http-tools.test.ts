...
function withTestServer(
  ...
  const closeServer = (server: http.Server): Promise<void> => new Promise<void>((done, fail) => {
  ...
  it('http_fetch supports regex/snippet filtering and returns links', async () => {
    ...
    const srv = await withTestServer((req, res) => {
      if (req.url === '/article') {
        ...
        res.end('<html><body><h1>Alpha keyword line</h1><a href="/next">next</a><p>Other text</p></body></html>');
        return;
      }
