/**
 * Bridge between AI Team agent/config types and @ai-team/permission.
 *
 * Converts Agent/FileTreeConfig into AccessContext/AccessRule,
 * and provides a factory to create a fully wired PermissionEngine.
 */
...
export interface CreatePermissionEngineOptions {
  ...
  /** Whether to scan the workspace for convention files (.gitignore, .perm, etc.). Default: true. */
  autoLoadWorkspaceConventions?: boolean;
}

/**
 * Factory: create a fully wired PermissionEngine from AI Team config data.
 *
 * Registers global context from FileTreeConfig, one context per agent,
 * and all built-in tool + command descriptors.
 */
export function createPermissionEngine(options: CreatePermissionEngineOptions): PermissionEngine {
