...
import { listWorkspaceFiles, type FlatFileEntry } from '@ai-team/fs';
import {
  analyzePermOverlap,
  parseAccessFile,
  type AccessRule,
...
function summarizeByExtension(files: readonly FileOwnershipEntry[]): FileResponsibilityByExtension[] {
...
function mergeUniquePatterns(...groups: ReadonlyArray<readonly string[]>): string[] {
  return [...new Set(groups.flat().map((pattern) => pattern.trim()).filter((pattern) => pattern.length > 0))];
}

async function mergeAgentAccessPatterns(
  workspaceRoot: string,
  agents: readonly Awaited<ReturnType<AgentManager['getAllAgents']>>[number][],
): Promise<Awaited<ReturnType<AgentManager['getAllAgents']>>> {
  return Promise.all(agents.map(async (agent) => {
    ...
    return {
      ...
      permissions: {
        ...
        create: mergeUniquePatterns(agent.permissions?.create ?? [], persistedPatterns.create),
        delete: mergeUniquePatterns(agent.permissions?.delete ?? [], persistedPatterns.delete),
      },
    };
  }));
}
...
function buildPairOverlaps(
  ...
  for (let index = 0; index < agentIds.length; index += 1) {
    ...
    for (let inner = index + 1; inner < agentIds.length; inner += 1) {
      ...
      const filesB = files.filter((file) => (ownershipByPath.get(file.path) ?? []).includes(agentB));
...
function buildAgentFocus(
  ...
  return {
    ...
    rights: createRightRecord((right) => {
      ...
      const responsibility = summary.agentResponsibilities.find((entry) => entry.agentId === agentId) ?? {
        agentId,
        fileCount: 0,
        lineCount: 0,
        byExtension: [],
...
export async function analyzeWorkspacePermissionOverlap(
  ...
  const agentIds = filterAgentIds(agents.map((agent) => agent.id).sort((a, b) => a.localeCompare(b)), options.agentId);
  ...
  const rights = createRightRecord((right) => {
    ...
    for (const file of files) {
      const owners = agentIds.filter((agentId) => rightsByAgent.get(agentId)?.get(file.path)?.has(right) ?? false);
