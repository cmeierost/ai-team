/**
 * Info command - render an agent's full profile / portfolio
 */
...
interface InfoRenderOptions {
  json?: boolean;
  openAvatar?: boolean;
  workspaceRoot?: string;
}

export async function renderAgentInfo(
  matches: Agent[],
  query: string,
  options: InfoRenderOptions = {}
): Promise<void> {
  if (matches.length === 0) {
    console.log(chalk.red(`No agent found matching "${query}".`));
    process.exit(1);
  }

  if (matches.length > 1) {
    console.log(chalk.yellow(`Multiple agents match "${query}":`));
  ...
  if (options.openAvatar) {
    if (agent.avatar?.type === 'url' && agent.avatar.url) {
      const avatarPath = resolve(options.workspaceRoot || process.cwd(), agent.avatar.url);
