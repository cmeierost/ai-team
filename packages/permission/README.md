...
File-path permission rights library — a layered policy engine for shell commands, tool calls, and direct path checks.
...
├── engine.ts         # PermissionEngine — main entry point
...
```typescript
import { PermissionEngine } from '@ai-team/permission';

const engine = new PermissionEngine({ workspaceRoot: '/workspace' });
...
```typescript
import { createPermissionEngine } from '@ai-team/core';
...
```bash
pnpm --filter @ai-team/permission build
pnpm --filter @ai-team/permission exec vitest run
```
