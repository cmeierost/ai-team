...
export type AgentRuleMap = Map<string, readonly AccessRule[]>;
...
function normalizePattern(pattern: string): string {
  const normalized = path.posix.normalize(pattern.replaceAll('\\', '/').trim());
  if (normalized === '.') {
    return '';
  }
...
function collectAgentPatternSets(rules: readonly AccessRule[]): AgentPatternSets {
