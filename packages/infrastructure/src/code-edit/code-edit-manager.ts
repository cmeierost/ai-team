...
export class CodeEditManager {
  ...
  async createProposal(
    ...
    // Validate file paths
    const pathValidation = this.validator.validateFilePaths(input.changes.map((c) => c.filePath));
  ...
  getTerminalDiffs(proposalId: string): string[] {
    ...
    return proposal.changes.map((change) => this.diffBuilder.formatForTerminal(change.diff));
  }
