...
export class TreeSitterManager {
  ...
  /**
   * Get the parser, lazily initializing WASM on first call.
   */
  ...
  async loadLanguage(languageName: string): Promise<void> {
    await this.getParser(); // ensure initialized
   ...
   */
  async getParserForLanguage(languageName: string): Promise<Parser> {
  ...
  async parse(sourceCode: string, languageName: string): Promise<Tree> {
    const parser = await this.getParserForLanguage(languageName);
