...
export class TeamGraphBuilder implements ITeamGraphBuilder {
  ...
  async buildGraph(
    ...
    switch (viewMode) {
      case 'hierarchy':
        await this.addHierarchyEdges(agents, edges);
        break;
  ...
  private async resolveManagerId(
    ...
    const exactMatch = await this.agentManager.getAgentAsync(reportsTo);
    if (exactMatch) {
      return { id: reportsTo };
    }
    ...
    const matches = await this.agentManager.resolveAgentAsync(reportsTo);
   ...
   */
  private async addHierarchyEdges(agents: Agent[], edges: GraphEdge[]): Promise<void> {
    for (const agent of agents) {
      if (agent.reportsTo) {
        const { id: managerId, error } = await this.resolveManagerId(agent.reportsTo);
  ...
  calculateHierarchicalLayout(graphData: GraphData): GraphData {
    ...
    const hasManager = new Set(
      edges.filter((e) => e.type === EdgeType.REPORTS_TO).map((e) => e.source)
    );
