...
import type { StoredProposal } from '@ai-team/core';
import { InfrastructureProposalStoreFactory, ProposalStore } from './proposal-store.js';
...
describe('ProposalStore round-trip', () => {
  ...
  it('factory creates proposal stores for a workspace root', () => {
    const workspaceRoot = createTempWorkspace();
    const factory = new InfrastructureProposalStoreFactory();

    const store = factory.create(workspaceRoot);
    store.save(makeProposal({ proposalId: 'factory-1' }));

    const persisted = new ProposalStore(workspaceRoot).loadAll();
    expect(persisted).toHaveLength(1);
    expect(persisted[0].proposalId).toBe('factory-1');
  });
});
