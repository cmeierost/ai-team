...
export class ChatStorage {
  ...
  private getChatFilePath(agentId: string): string {
    return path.join(this.workspaceRoot, '.ai-team', 'private', 'chats', `${agentId}.jsonl`);
  }
  ...
  async saveMeetingSummary(summary: MeetingSummary): Promise<string> {
    ...
    const filePath = path.join(this.workspaceRoot, '.ai-team', 'meetings', fileName);
  ...
  async loadMeetingSummaries(): Promise<string[]> {
    ...
    try {
      ...
      return files.filter((f) => f.endsWith('.md')).map((f) => path.join(meetingsDir, f));
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        return [];
      }
   ...
   */
  async saveMessages(agentId: string, messages: ChatMessage[]): Promise<void> {
    const filePath = this.getChatFilePath(agentId);
    await fs.mkdir(path.dirname(filePath), { recursive: true });
