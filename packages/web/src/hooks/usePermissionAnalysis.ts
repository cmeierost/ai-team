...
const CATEGORY_ORDER: FileTypeCategory[] = [
  'code',
  'documentation',
  'configuration',
  'tests',
...
function summarizeFilesByEnding(
  files: readonly PermissionOverlapFileOwnershipEntry[]
): FileEndingSummary[] {
...
function summarizeFilesByCategory(
  files: readonly PermissionOverlapFileOwnershipEntry[]
): FileTypeSummary[] {
  const aggregate = new Map<
    FileTypeCategory,
    { fileCount: number; lineCount: number; extensions: Set<string> }
  >();
...
function buildEffectiveFileTypeGroups(report: Extract<PermissionOverlapReport, { kind: 'files' }>) {
  const configured: Record<string, FileTypeGroupLike> =
    report.fileTypeGroups && Object.keys(report.fileTypeGroups).length > 0
      ? report.fileTypeGroups
      : DEFAULT_FILE_TYPE_GROUPS;
  const groups: Array<{ id: string; label: string; patterns: string[] }> = Object.entries(
    configured
  )
    .map(([id, group]) => ({
      id,
...
function makeFileMatcher(
  ...
  const selectedExactPatterns = new Set(
    (selected?.patterns ?? []).map((pattern) => pattern.toLowerCase())
  );
  return (file) => {
    const normalizedPath = file.path.replaceAll('\\', '/').toLowerCase();
    const matching = allBySpecificity.filter((entry) =>
      minimatch(normalizedPath, entry.pattern, { dot: true })
    );
    ...
    return (
      best.groupId === selectedFileTypeGroupId ||
      matching.some(
        (entry) =>
          entry.groupId === selectedFileTypeGroupId &&
...
function applyFileTypeFilterToReport(
  ...
  const filteredRights = RIGHTS.reduce(
    (acc, right) => {
      ...
      const agentResponsibilities = report.agentIds
        .map((agentId) => {
  ...
  const outsideDefaultContextByAgent = report.outsideDefaultContextByAgent.map((entry) => ({
    ...
    rights: {
      ...
      read: {
        ...
        lineCount: entry.rights.read.files
          .filter(matcher)
      ...
      write: {
        ...
        lineCount: entry.rights.write.files
          .filter(matcher)
          .reduce((sum, file) => sum + file.lineCount, 0),
      },
      list: {
        ...
        lineCount: entry.rights.list.files
          .filter(matcher)
          .reduce((sum, file) => sum + file.lineCount, 0),
      },
    },
...
function intersectGloballyUncoveredFiles(
  report: Extract<PermissionOverlapReport, { kind: 'files' }>
) {
...
function buildRegions(
  ...
  for (const right of RIGHTS) {
    for (const pair of report.rights[right].pairs) {
      ...
      const existing: RegionAggregate = aggregate.get(key) ?? {
        ...
        rightSharedFiles: {} as Partial<
          Record<PermissionRight, PermissionOverlapFileOwnershipEntry[]>
        >,
      };
      ...
      existing.rightFolderCounts[right] = new Set(
        pair.sharedFiles.map((file) => getParentDirectory(file.path))
      ).size;
  ...
  return [...aggregate.entries()]
    .map(([id, entry]) => {
      ...
      const rightFileEndingSummary = RIGHTS.reduce(
        (acc, right) => {
      ...
      const rightFileTypeSummary = RIGHTS.reduce(
        (acc, right) => {
          const filesForRight = dedupeFiles(entry.rightSharedFiles[right] ?? []);
      ...
      return {
        ...
        rightSharedFiles: RIGHTS.reduce(
          (acc, right) => {
...
function buildUncoveredSuggestion(
  files: readonly PermissionOverlapFileOwnershipEntry[]
): PermissionSuggestion[] {
    ...
    fileTypeSummary: [
      {
        ...
        extensions: [summary.extension],
      },
    ],
  }));
}

function buildWriteOverlapSuggestions(
  ...
  return regions
    ...
    .map((region, index) => ({
      ...
      fileScope: region.fileEndingSummary
        .slice(0, 3)
        .map((entry) => `${entry.extension} (${entry.fileCount} files)`),
      fileTypeSummary: region.fileTypeSummary,
    }));
}

function buildSparseOwnershipSuggestions(
  report: Extract<PermissionOverlapReport, { kind: 'files' }>
): PermissionSuggestion[] {
  const candidates = report.rights.write.agentResponsibilities
    .filter((entry) => entry.fileCount > 0)
    .sort(
      (left, right) => left.fileCount - right.fileCount || left.agentId.localeCompare(right.agentId)
    )
    .slice(0, 2);
  ...
  return candidates.map((entry) => ({
    ...
    affectedRights: ['write'],
    fileScope: entry.byExtension
      .slice(0, 3)
      .map((item) => `${item.extension} (${item.fileCount} files)`),
...
function buildRightUncovered(
  report: Extract<PermissionOverlapReport, { kind: 'files' }>
): Record<PermissionRight, PermissionRightUncoveredSummary> {
...
function buildTotalContextByRight(
  report: Extract<PermissionOverlapReport, { kind: 'files' }>
): Record<PermissionRight, number> {
  return RIGHTS.reduce(
    (acc, right) => {
...
function buildAgentResponsibilities(
  report: Extract<PermissionOverlapReport, { kind: 'files' }>
): Record<string, PermissionAgentResponsibilitySummary> {
...
export function buildPermissionAnalysisView(
  report: PermissionOverlapReport,
  selectedFileTypeGroupId = 'all'
): PermissionAnalysisView {
  ...
  const effectiveSelectedGroup = fileTypeGroups.some(
    (group) => group.id === selectedFileTypeGroupId
  )
    ? selectedFileTypeGroupId
    : 'all';
  const matcher = makeFileMatcher(effectiveSelectedGroup, fileTypeGroups);
  const effectiveReport =
    effectiveSelectedGroup === 'all' ? report : applyFileTypeFilterToReport(report, matcher);
  ...
  const defaultContextByRight = {
    read:
      effectiveReport.rights.read.totalFiles - effectiveReport.rights.read.uncoveredFiles.length,
    write:
      effectiveReport.rights.read.totalFiles - effectiveReport.rights.read.uncoveredFiles.length,
    list:
      effectiveReport.rights.read.totalFiles - effectiveReport.rights.read.uncoveredFiles.length,
  } satisfies Record<PermissionRight, number>;
  const defaultReadContextFileCount =
    effectiveReport.rights.read.totalFiles - effectiveReport.rights.read.uncoveredFiles.length;
  const defaultReadContextLineCount = [
  ...
  const workspaceCodeFiles = workspaceReadFiles.filter(
    (file) => inferFileTypeCategory(file.extension, file.path) === 'code'
  );
  const workspaceDocumentationFiles = workspaceReadFiles.filter(
    (file) => inferFileTypeCategory(file.extension, file.path) === 'documentation'
  );
  const workspaceBinaryFiles = workspaceReadFiles.filter((file) =>
    isBinaryExtension(file.extension)
  );
  const workspaceCodeUncoveredFiles = effectiveReport.rights.read.uncoveredFiles.filter(
    (file) => inferFileTypeCategory(file.extension, file.path) === 'code'
  );
  const workspaceDocumentationUncoveredFiles = effectiveReport.rights.read.uncoveredFiles.filter(
    (file) => inferFileTypeCategory(file.extension, file.path) === 'documentation'
  );
  const workspaceBinaryUncoveredFiles = effectiveReport.rights.read.uncoveredFiles.filter((file) =>
    isBinaryExtension(file.extension)
  );
  ...
  const workspaceBinaryUncoveredByRight = buildUncoveredByRight(effectiveReport, (file) =>
    isBinaryExtension(file.extension)
  );
...
export function usePermissionAnalysis(options: UsePermissionAnalysisOptions = {}) {
  ...
  const query = useQuery<PermissionOverlapReport>({
    ...
    queryFn: () =>
      client.access.analyzeOverlap({ mode: 'files' }) as Promise<PermissionOverlapReport>,
    staleTime: 30_000,
  ...
  const view = useMemo(
    () =>
      query.data
        ? buildPermissionAnalysisView(query.data, options.selectedFileTypeGroupId ?? 'all')
        : undefined,
...
export function filterRegionsForAgent(
