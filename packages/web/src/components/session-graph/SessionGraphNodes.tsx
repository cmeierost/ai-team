  ...
  return (
    <>
      ...
      <button
        ...
        onClick={nodeData.isGhost ? undefined : () => nodeData.onSelect(nodeData.session.sessionId, nodeData.targetAgentId, nodeData.inboundHandoffId)}
      >
        {nodeData.isGhost ? (
          <span className="sg-session-bar-ghost-label">deleted</span>
        ) : (
          <>
            <span className="sg-session-bar-msgs">{nodeData.session.messageCount}msg</span>
