...
export const filesAllowCliMetadata: CliCommandMetadata = {
  key: 'files.allow',
  command: 'allow <path>',
  parentKey: 'files',
  description:
...
export const filesAllowCommandDefinition = createResolverCommandDefinition(
  'filesAllow',
          ...
          payload.path,
          {
            ...
            confirmUserApproval: async () =>
              confirmGovernanceAction(
                ...
                context,
                `Approve access_allow by ${requestedBy} for agent '${payload.agent}', mode '${mode}', path '${payload.path}'?`
              ),
          },
          mode
        );
